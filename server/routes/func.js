...
module.exports = {
    ...
    {
       con.query("SELECT adminname as username,full_name as name,email FROM admin where status=1 and admin_id=?",[req.session.adminid],function(err,result,fields)
      {
        if(err)
        {
          console.log(err);
        ...
        {
          // return(result);
           res.json({'success':true,'msg':'admin home page','data':result})        }
        else
        { 
          res.json({"success":true,'msg':'something wrong'});  
          // return ({});  
        }      
      });
    },
